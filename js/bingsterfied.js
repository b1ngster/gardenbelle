...
/*
navOpener called via onlick on mobile nav
timesClicked: opens and closes apps depending on timesClicked.
declared outside of function scope so not over written.
*/
...
function openNav(elem = null) {
	
	/* tween sidebar to 70% of display 
	   ensure that the navigation is not invisible
	*/
	...
	var pageoffsetwidth = document.documentElement.clientWidth * 0.7;
	console.log(pageoffsetwidth);
	tween(sidebar, {width: pageoffsetwidth} ,0.5, Quad_easeIn);
...
function closeNav() {
	...
	/*function closes navigation hiding the side nav to prevent
	  flash of unstyled elements */
	var sidebar = document.getElementById("sidebar");
	var sidenav = document.getElementById("sidenav");
	...
	tween(sidebar, {width: 0} ,0.5,  Quad_easeIn);
...
function Quad_easeInOut (t, b, c, d) { if ((t/=d/2) < 1) return c/2*t*t*t*t + b; return -c/2 * ((t-=2)*t*t*t - 2) + b; }				/* Simple JavaScript Tween v 0.2  - http://mattshaw.org/projects/simple-javascript-tweening 
 * 
 * [Arguments]
 * o: Target element
 * props: key/values object of props to tween
